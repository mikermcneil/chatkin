  ...
  fn: function (inputs, exits, env) {
    ...
    .exec({
      ...
      success: function(loggedInUserId) {
        ...
        User.findOne({ id: loggedInUserId })
        .exec(function(err, thisUser){
          if (err) { return exits.error(err); }
          if (!thisUser) { return exits.error(new Error('The requesting user is logged in as user `'+loggedInUserId+'`, but no such user exists in the database.  This should never happen!')); }
